this.ai_attack_big_reap <- this.inherit("scripts/ai/tactical/behavior", {
	m = {
		TargetTile = null,
		PossibleSkills = [
			"actives.big_reap"
	...
	function onExecute( _entity )
	{
		...
		if (this.m.TargetTile != null && this.m.TargetTile.IsOccupiedByActor)
		{
			...
			{
				this.logInfo("* " + _entity.getName() + ": Using Big Reap against " + this.m.TargetTile.getEntity().getName() + "!");
			}
