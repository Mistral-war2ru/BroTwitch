this.big_disarm_skill <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
		this.m.ID = "actives.big_disarm";
		this.m.Name = "Disarm";
	...
	function getTooltip()
	{
		...
		if (this.m.HitChanceBonus != 0)
		{
			ret.push({
				...
				text = "Have [color=" + this.Const.UI.Color.NegativeValue + "]" + this.m.HitChanceBonus + "%[/color] hit chance"
			});
		}
		...
		ret.push({
			...
			text = "Have [color=" + this.Const.UI.Color.PositiveValue + "]100%[/color] disarm chance"
		});
		return ret;
	}
