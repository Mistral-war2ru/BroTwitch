this.big_strike_skill <- this.inherit("scripts/skills/skill", {
	m = {
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "Дистанция [color=" + this.Const.UI.Color.PositiveValue + "]3" + "[/color] клетки"
		});
		...
		if (this.m.ApplyAxeMastery)
		{
			if (!this.getContainer().getActor().getCurrentProperties().IsSpecializedInAxes)
			{
				ret.push({
					...
					text = "Have [color=" + this.Const.UI.Color.NegativeValue + "]-15%[/color] hit chance on close target"
				});
			}
		}
		else
		{
			ret.push({
				...
				type = "text",
				icon = "ui/icons/hitchance.png",
				text = "Have [color=" + this.Const.UI.Color.PositiveValue + "]+5%[/color] hit chance"
			});
