this.big_reap_skill <- this.inherit("scripts/skills/skill", {
	m = {},
	function create()
	{
		this.m.ID = "actives.big_reap";
		this.m.Name = "Reap";
	...
	function getTooltip()
	{
		...
		ret.push({
			...
			text = "Distance [color=" + this.Const.UI.Color.PositiveValue + "]3" + "[/color] tiles"
		});
		...
		if (!this.getContainer().getActor().getCurrentProperties().IsSpecializedInPolearms)
		{
			ret.push({
				...
				text = "Have [color=" + this.Const.UI.Color.NegativeValue + "]-15%[/color] hit chance to close target"
			});
		}
