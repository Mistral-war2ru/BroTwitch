this.twitch_barbarian_boss_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Destination = null,
		Dude = null,
		Reward = 0,
	...
	function create()
	{
		...
		this.m.Type = "contract.twitch_barbarian_boss";
		this.m.Name = "Defeat North Guardian";
		...
		this.m.States.push({
			...
			function start()
			{
				this.Contract.m.BulletpointsObjectives = [
					"Defeat North Guardian in camp " + this.Flags.get("DestinationName") + ", you can find it on %direction% from %origin%"
				];
		...
		this.m.States.push({
			...
			function onDestinationAttacked( _dest, _isPlayerAttacking = true )
			{
					...
					else
					{
						...
						gt.tnf_debug.add_boss5(p, gt.tnf_debug.calc_diff(), "North Guardian");
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "[img]gfx/ui/events/event_20.png[/img]{%employer% shakes his head tiredly.%SPEECH_ON%In these dark times I meet many people, but when night falls I fear uninvited guests. Lately we have been constantly attacked by barbarians. Their leader calls himself North Guardian. He says that this is their land, and that they do not want us here. What an impudent fellow! However, each time their actions become more and more terrible. The people of our settlement %townname% would be very happy if this band of barbarians suddenly disappeared by chance, and then they would finally leave us alone.%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
	...
	function onHomeSet()
	{
		...
		this.logDebug("North Guardian contract in city " + this.m.Home.getName());
	}
