this.twitch_snake_boss_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Destination = null,
		Dude = null,
		Reward = 0,
	...
	function create()
	{
		...
		this.m.Type = "contract.twitch_snake_boss";
		this.m.Name = "Defeat Snake charmer";
		...
		this.m.States.push({
			...
			function start()
			{
				this.Contract.m.BulletpointsObjectives = [
					"Defeat Snake charmer in camp " + this.Flags.get("DestinationName") + ", you can find it on %direction% from %origin%"
				];
		...
		this.m.States.push({
			...
			function onDestinationAttacked( _dest, _isPlayerAttacking = true )
			{
					...
					else
					{
						...
						gt.tnf_debug.add_boss3(p, gt.tnf_debug.calc_diff(), "Snake charmer");
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "[img]gfx/ui/events/event_20.png[/img]{%employer% shakes his head tiredly.%SPEECH_ON%In these dark times I meet many people, but when night falls I fear uninvited guests. Lately this crazy hermit with his huge snakes has been wandering around here and constantly frightening the locals. Every day his actions bring more and more problems. The people of our settlement %townname% would be very happy if this hermit suddenly disappeared by chance, and then we would finally have peace.%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
	...
	function onHomeSet()
	{
		...
		this.logDebug("Snake charmer contract in city " + this.m.Home.getName());
	}
