this.twitch_skeleton_boss_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Destination = null,
		Dude = null,
		Reward = 0,
	...
	function create()
	{
		...
		this.m.Type = "contract.twitch_skeleton_boss";
		this.m.Name = "Defeat Bonedance";
	...
	function start()
	{
		local banditcamp = this.World.FactionManager.getFactionOfType(this.Const.FactionType.Undead).getNearestSettlement(this.m.Home.getTile());
		...
		this.m.States.push({
			...
			function start()
			{
				this.Contract.m.BulletpointsObjectives = [
					"Defeat Bonedance in camp " + this.Flags.get("DestinationName") + ", you can find it on %direction% from %origin%"
				];
		...
		this.m.States.push({
			...
			function onDestinationAttacked( _dest, _isPlayerAttacking = true )
			{
					...
					else
					{
						...
						gt.tnf_debug.add_boss4(p, gt.tnf_debug.calc_diff(), "Bonedance");
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "[img]gfx/ui/events/event_20.png[/img]{%employer% shakes his head tiredly.%SPEECH_ON%In these dark times I meet many people, but when night falls I fear uninvited guests. Lately I have often heard from my people about stories of a certain dancing skeleton. At first I did not believe it, but not now, when there have already been victims. With each passing day his actions become more and more unpredictable. The people of our settlement %townname% would be very happy if he suddenly disappeared by chance, and then we could finally rest easy.%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
	...
	function onHomeSet()
	{
		...
		this.logDebug("Bonedance contract in city " + this.m.Home.getName());
	}
