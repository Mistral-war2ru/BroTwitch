this.twitch_nomad_boss_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Destination = null,
		Dude = null,
		Reward = 0,
	...
	function create()
	{
		...
		this.m.Type = "contract.twitch_nomad_boss";
		this.m.Name = "Defeat Desert warrior";
		...
		this.m.States.push({
			...
			function start()
			{
				this.Contract.m.BulletpointsObjectives = [
					"Defeat Desert warrior in camp " + this.Flags.get("DestinationName") + ", you can find it on %direction% from %origin%"
				];
		...
		this.m.States.push({
			...
			function onDestinationAttacked( _dest, _isPlayerAttacking = true )
			{
					...
					else
					{
						...
						gt.tnf_debug.add_boss0(p, gt.tnf_debug.calc_diff(), "Desert warrior");
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "[img]gfx/ui/events/event_20.png[/img]{%employer% shakes his head tiredly.%SPEECH_ON%In these dark times I meet many people, but when night falls I fear uninvited guests. Lately a bandit group has appeared here. They are not like the usual rabble, they have well-organized and trained gunners. Every day their actions bring more and more problems. The people of our settlement %townname% would be very happy if this group suddenly disappeared by chance, and then they would finally leave us alone.%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
	...
	function onHomeSet()
	{
		...
		this.logDebug("Desert warrior contract in city " + this.m.Home.getName());
	}
